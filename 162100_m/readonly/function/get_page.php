...
';
  ...
  if ($totallists > $pagesize) {
    ...
    $text .= ($totalpages > $pagestep ? '<kbd><input type="text" name="pagego" id="pagego" size="3" title="按回车直接跳页" onblur="$(\'pagego\').value=this.value;" onkeydown="if(event.keyCode==13&&!isNaN(this.value)&&this.value>=1&&this.value<='.$totalpages.'){window.location=\''.$mpurl_before.'\'+this.value+\''.$mpurl_after.'&pn='.$_GET['pn'].'\';return false;}" /><!-- button type="button" id="gopage" onclick="if(!isNaN($(\'pagego\').value) && $(\'pagego\').value>=1 && $(\'pagego\').value<='.$totalpages.'){window.location=\''.$mpurl_before.'\'+$(\'pagego\').value+\''.$mpurl_after.'&pn='.$_GET['pn'].'\';return false;}">go</button --></kbd>' : '');
    $text .='</div>';
    if ($code_other == '') {
