...
if ($_GET['act'] == 'punished') {
  $where_eval = '2';
  $list_title = '"<a href=\"?get=member_check_reg\">待审批注册用户列表</a> <a class=\"list_title_in\">黑名单（".$n."）</a>'.$search_r.' <a href=\"?get=member_check_reg&act=abnormal\">冻结</a> <a href=\"?get=member_check_reg&act=warn\">观察</a>"';
  $choose = '<button name="act" type="button" onclick="chk(this.form,this,\'out_punished\')">解除</button>';
} elseif ($_GET['act'] == 'abnormal') {
  $where_eval = '3';
  $list_title = '"<a href=\"?get=member_check_reg\">待审批注册用户列表</a> <a href=\"?get=member_check_reg&act=punished\">黑名单</a> <a class=\"list_title_in\">冻结（".$n."）</a>'.$search_r.' <a href=\"?get=member_check_reg&act=warn\">观察</a>"';
  $choose = '<button name="act" type="button" onclick="chk(this.form,this,\'out_punished\')">解除</button>';
} elseif ($_GET['act'] == 'warn') {
  $where_eval = '4';
  $list_title = '"<a href=\"?get=member_check_reg\">待审批注册用户列表</a> <a href=\"?get=member_check_reg&act=punished\">黑名单</a> <a href=\"?get=member_check_reg&act=abnormal\">冻结</a> <a class=\"list_title_in\">观察（".$n."）</a>'.$search_r.'"';
  $choose = '<button name="act" type="button" onclick="chk(this.form,this,\'out_punished\')">解除</button>';
} else {
  $where_eval = '1';
  $list_title = '"<a class=\"list_title_in\">待审批注册用户列表（".$n."）</a>'.$search_r.' <a href=\"?get=member_check_reg&act=punished\">黑名单</a> <a href=\"?get=member_check_reg&act=abnormal\">冻结</a> <a href=\"?get=member_check_reg&act=warn\">观察</a>"';
...
if ($sql['db_err'] == '') {
  ...
  $result = @mysql_query('SELECT COUNT(id) AS total FROM '.$sql['pref'].''.$sql['data']['承载成员档案的表名'].' WHERE'.$eval.' check_reg="'.$where_eval.'"', $db);
...
@ require ('readonly/function/get_page.php');
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="1" id="ad_table">
    <tr>
      <th width="12">&nbsp;</th>
      <th>用户名<select name="order" id="order" onchange="location.href=\'?get=member_check_reg&p='.$p.'&username='.urlencode($_GET['username']).'&pn='.$_GET['pn'].'&order=\'+this.value+\'\';">
...
';
    ...
    $result = @mysql_query('SELECT * FROM '.$sql['pref'].''.$sql['data']['承载成员档案的表名'].' WHERE'.$eval.' check_reg="'.$where_eval.'" ORDER BY id DESC LIMIT '.$seek.','.$web['pagesize'].'', $db);
    ...
    $text .= get_page_foot($n, $web['pagesize'], $p, '?get=member_check_reg&username='.urlencode($_GET['username']).'&act='.$_GET['act'].'&pn='.$_GET['pn'].'&order='.$_GET['order'].'&p=');
...
?>
<h5><?php echo eval('return '.$list_title.';'); ?></h5>
