...
';
    ...
    if (preg_match('/<p class="slider_whicon_w"><img[^\>]*src="([^\"\>]+)"[^\>]*alt="([^\"\>]+)"[^\>]*><\/p>[\s\n\r]*<div class="slider_degree">([^\<]*)<\/div>.+<div class="slider_warn">(.*)id="slider_arr_l"/isU', $W_FR, $matches_kq)) {
	  $weather .= '<span id="w_today" title="当前"><span class="w_img"><img src="'.get_w_img(preg_replace('/icons_\d+_wt/i', 'icons_42_yl', $matches_kq[1])).'" style="width:24px;height:24px;" /></span> <span class="w_qingkuang">'.$matches_kq[2].'</span> <span class="w_wendu'.(floatval($matches_kq[3])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches_kq[3].'</span></span> ';
      ...
      if (preg_match('/<p class="slider_warn_map">.+<\/p>/isU', $matches_kq[4], $matches_kongqi)) {
        $weather .= '<span id="w_kq" title="空气：'.$kongqi.'">'.$matches_kongqi[0].'</span>';
      }
...
/*
    ...
    if (preg_match('/<p class="wt_fc_c0_i_day wt_fc_c0_i_today">(.+)<\/div>/isU', $W_FR, $matches_kq)) {
      if (preg_match('/<img[^\>]*src="([^\"\>]+)"[^\>]*alt="([^\"\>]+)".+<img[^\>]*src="([^\"\>]+)"[^\>]*alt="([^\"\>]+)".+<p class="wt_fc_c0_i_temp">([^\<\>]+)\/([^\<\>]+)<\/p>/isU', $matches_kq[1], $matches)) {
	    $weather .= ' <span id="w_today" title="白天">白天 <span class="w_img2"><img src="'.get_w_img($matches[1]).'" style="width:24px;height:24px;" /></span> <span class="w_qingkuang">'.$matches[2].'</span></span> <span title="白天最高" class="w_wendu'.(floatval($matches[5])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches[5].'';
        $weather .= '</span>';
//        $weather .= ' ～ ';
...
//        $weather .= '<!--span class="w_xiangqing" title="未来详情">︾</span-->';
    ...
    if ($runget1 == false) {
      ...
      if (is_array($matches) && is_array($matches2)) {
	    $weather .= ' <span id="w_moday" title="夜间">夜间<span class="w_img2"><img src="'.get_w_img($matches[1]).'" style="width:24px;height:24px;" /></span> <span class="w_qingkuang">'.$matches[2].'</span></span> <span title="夜间最低" class="w_wendu'.(floatval($matches[3])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches[3].'';
        $weather .= '</span>';
