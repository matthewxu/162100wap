...
';
    ...
    if (preg_match('/<div\s+class="today"\s+style="display:block;">(.+)<\/div>/isU', $W_FR, $matches_kq)) {
      ...
      if (count($matches1[1]) == 2) {
	    $weather .= $today = '<span id="w_today" title="白天"><span class="w_img"><img src="'.get_w_img($matches1[1][0]).'" style="width:34px !important; height:27px !important;" /></span><span class="w_qingkuang">'.$matches2[1][0].'</span><span class="w_wendu'.(floatval($matches3[1][0])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches3[1][0].'</span>';
        $weather .= '</span>';
        $weather .= ' ～ ';
	    $weather .= $night = '<span id="w_moday" title="夜间"><span class="w_img"><img src="'.get_w_img($matches1[1][1]).'" style="width:34px !important; height:27px !important;" /></span><span class="w_qingkuang">'.$matches2[1][1].'</span><span class="w_wendu'.(floatval($matches3[1][1])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches3[1][1].'</span>';
        $weather .= '</span>';
	    ...
	    $weather .= $night = '<span id="w_today" title="夜间"><span class="w_img"><img src="'.get_w_img($matches1[1][0]).'" style="width:34px !important; height:27px !important;" /></span>夜晚:<span class="w_qingkuang">'.$matches2[1][0].'</span><span class="w_wendu'.(floatval($matches3[1][0])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches3[1][0].'</span>';
        $weather .= '</span>';
        if (preg_match('/<div class="day">[\s\n\r]*<div class="date">[\s\n\r]*[\s\n\r]*明\s*天[\s\n\r]*<\/div>[\s\n\r]*<div class="week">.*<\/div>[\s\n\r]*<div class="wicon">[\s\n\r]*<img [^>]*src="(.+)"[^>]*>[\s\n\r]*<\/div>[\s\n\r]*<div class="wdesc">(.+)<\/div>[\s\n\r]*<div class="temp">(.+)<\/div>/isU', $W_FR, $matches_kq2)) {
          $weather .= ' &nbsp; ';
	      $weather .= $moday = '<span id="w_moday" title="明天"><span class="w_img"><img src="'.get_w_img($matches_kq2[1]).'" style="width:34px !important; height:27px !important;" /></span>明天:<span class="w_qingkuang">'.$matches_kq2[2].'</span><span class="w_wendu'.(floatval($matches_kq2[3])>0 ? ' w_wendu_ls':' w_wendu_lx').'">'.$matches_kq2[3].'</span>';
          $weather .= '</span>';
        }
        $weather .= '<!--span class="w_xiangqing" title="未来详情">︾</span-->';
        $weather .= '</a>
...
function getCITY2($city) {
  ...
  if ($city_arr_file = file_get_contents($GLOBALS['WEATHER_DATA'].'readonly/weather/'.$web['weather_from'].'/getweather_seek.php')) {
    if (preg_match('/\''.preg_quote($city, '/').'\|([a-z\-]+\|[A-Z]+)\'/', $city_arr_file, $m)) {
...
if ($_GET['city']) {
  if ($city_arr_file = file_get_contents($GLOBALS['WEATHER_DATA'].'readonly/weather/'.$web['weather_from'].'/getweather_seek.php')) {
    if (preg_match('/\''.preg_quote($_GET['city'], '/').'\|([a-z\-]+\|[A-Z]+)\'/', $city_arr_file, $m)) {
...
if (empty($city)) {
  if ($_COOKIE['weathercity']) {
    ...
    if ($city_arr_file = file_get_contents($GLOBALS['WEATHER_DATA'].'readonly/weather/'.$web['weather_from'].'/getweather_seek.php')) {
      if (preg_match('/\''.preg_quote($city_tmp, '/').'\|([a-z\-]+\|[A-Z]+)\'/', $city_arr_file, $m)) {
